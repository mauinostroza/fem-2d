"""Modo lineal: placa con agujero (SolidsPy). Cuerpo original de `app.py`
antes del router de S6, movido tal cual a `render()` — sin cambios de
lógica, salvo namespacear la clave de `st.session_state` para convivir
con el modo de hormigón armado (`ui/rc_section.py`)."""
...
SESSION_KEY = "linear:last_result"
...
def render() -> None:
    ...
    if submitted:
        try:
            ...
            st.session_state[SESSION_KEY] = (mesh, result, hole_radius, admissible_stress)
    ...
    if SESSION_KEY in st.session_state:
        mesh, result, hole_radius, admissible_stress = st.session_state[SESSION_KEY]
